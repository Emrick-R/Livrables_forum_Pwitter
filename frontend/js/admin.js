...
async function chargerTopicsAdmin() {
    ...
    const liste  = Array.isArray(topics) ? topics : (topics.topics || [])
...
function creerLigneTopic(topic) {
    ...
    titre.innerHTML = `<a href="topic.html?id=${topic.id}" style="color:var(--bleu);font-weight:600">${topic.titre}</a>`

    const auteur = document.createElement('td')
    auteur.textContent = topic.pseudo || topic.auteur || '—'

    const statut = document.createElement('td')
    statut.innerHTML = `
        <select onchange="changerStatutTopic(${topic.id}, this.value)">
            <option value="ouvert"  ${topic.statut === 'ouvert'  ? 'selected' : ''}>Ouvert</option>
            <option value="ferme"   ${topic.statut === 'ferme'   ? 'selected' : ''}>Fermé</option>
            <option value="archive" ${topic.statut === 'archive' ? 'selected' : ''}>Archivé</option>
        </select>
    `
    ...
    actions.innerHTML = `
        <div class="admin-actions">
            <button class="btn-danger" onclick="supprimerTopicAdmin(${topic.id}, this)">Supprimer</button>
        </div>
    `
...
async function chargerUtilisateurs() {
    ...
    const liste = Array.isArray(utilisateurs) ? utilisateurs : (utilisateurs.users || [])
...
function creerLigneUtilisateur(u) {
    ...
    const estBanni = u.banni === true || u.banni === 1

    tr.innerHTML = `
        <td><strong>${u.pseudo}</strong></td>
        <td style="color:var(--texte-doux)">${u.email}</td>
        <td style="color:var(--texte-doux)">${u.role || 'utilisateur'}</td>
        <td>
            ...
            <div class="admin-actions">
                ${u.role !== 'admin' ? `
                    <button
                        ...
                        onclick="toggleBan(${u.id}, ${estBanni}, this)"
                    >
